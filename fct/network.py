...
def generate_batch(batch_size, batch_no, adjusted_train_count, train_dict,\
    ...
    batch_input_indices1 = np.ndarray(shape=(batch_size, 1), dtype=np.int32)
    batch_input_indices2 = np.ndarray(shape=(batch_size, 1), dtype=np.int32)
...
train_count = len(train_count.keys())
...
with tf.Session(graph=graph) as session:
    ...
    for step in xrange(num_steps):
        ...
        batch_output_indices,\
            = generate_batch(batch_size, batch_no, train_count, train_dict,\
                    vocab_dict,reverse_vocab_dict)

        feed_dict = {train_input_feature_set1: batch_features1, \
                        train_input_feature_set2: batch_features2, \
                        train_labels_word1: batch_input_indices1, \
                        train_labels_word2: batch_input_indices2, \
                        train_output_labels: batch_output_indices}
